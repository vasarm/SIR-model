    ...
    for i in range(1, height+1):
        for j in range(1, width+1):
            if copy_matrix[i, j] == 1:
                ...
                matrix[i, j] = np.random.choice(
                    [0, 2], size=1, p=[prob, 1-prob])
...
ill_h = np.random.randint(low=0, high=height-1, size=1)
ill_w = np.random.randint(low=0, high=width-1, size=1)
